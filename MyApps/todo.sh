...
# Exctracted from the Bash-Snippets git hub folder: https://github.com/alexanderepstein/Bash-Snippets/
flag=""
  ...
  if [[ $currentVersion == "" || $repositoryName == "" || $githubUserName == "" || $nameOfInstallFile == "" ]]; then
    ...
    if [[ "$latestVersion" != "$currentVersion" ]]; then
      ...
      if [[ "$answer" == [Yy] ]]; then
        ...
        git clone -q "https://github.com/$githubUserName/$repositoryName" && touch .BSnippetsHiddenFile || {
          echo "Failure!"
          exit 1
        } &
        ...
        rm -r -f $repositoryName || {
          echo "Permissions Error: update succesfull but cannot delete temp files located at ~/$repositoryName delete this directory with sudo"
          exit 1
        }
      else
        exit 1
...
if [[ $# == "0" ]]; then
  ...
  if [[ $1 == "clear" ]]; then
    ...
    usage
    exit 0
  elif [[ $flag == "clear" || $1 == "clear" ]]; then
    clearAllTasks || exit 1
  elif [[ $flag == "get" || $1 == "list" || $1 == "get" ]]; then
    getTasks || exit 1
  else {
    echo "Error: the argument $1 is not valid"
    exit 1
  }; fi
else
  if [[ $flag == "add" || $1 == "add" ]]; then
    addTask "${*:2}" && getTasks || exit 1
  elif [[ $flag == "remove" || $1 == "remove" ]]; then
    removeTask "${*:2}" && getTasks || exit 1
  else {
    usage
    exit 1
  }; fi
